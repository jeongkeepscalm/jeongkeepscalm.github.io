...
> * data : **new FormData(), $("#listForm").serialize()** 로 데이터를 지정해주면, url?id=1&name=ojg 와 같이 **쿼리파라미터 형식**으로 서버에 값을 보낸다 ( json 형식 x ) <br/>

* **@RestController(controller) , @ResponseBody(method)**
	해당 컨트롤러나 메서드에서 반환되는 값들을 JSON 형태로 변환하여 응답을 보낸다. <br/>
	return objectMapper.writeValueAsString(object); <br/>

* **new FormData(), $("#listForm").serialize();**
...
// 사용개선 코드 
async function apiReq(url, type, body, headers = {}) {
